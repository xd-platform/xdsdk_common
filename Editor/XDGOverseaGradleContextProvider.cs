...
#region Gradle Content String
        ...
        private const string GMS_ADS_ID_DEP = "    implementation 'com.google.android.gms:play-services-ads-identifier:18.0.1'";
        private const string AD_BILL_DEP = "    implementation 'com.android.billingclient:billing:7.0.0'";
        ...
        private HashSet<string> FirebaseIncludeBaseAGP7 = new HashSet<string>()
        {
            GMS_CLASSPATH_AGP_7,
            FIREBASE_CRASH_CLASSPATH_AGP_7
        };
        ...
        private void DeleteMainOldFirebaseContent(string pluginsFolder)
        {
            ...
            var baseProjectTemplate = Path.Combine(pluginsFolder, "baseProjectTemplate.gradle");
            if (!File.Exists(baseProjectTemplate))
            {
                baseProjectTemplate = Path.Combine(pluginsFolder, "baseProjectTemplate.gradle.DISABLED");
            }

            if (!File.Exists(baseProjectTemplate)) return;
            writerHelper = new XDGScriptHandlerProcessor(baseProjectTemplate);
            
            foreach (var dep in FirebaseIncludeBase)
            {
                writerHelper.Delete(dep);
            }
            foreach (var dep in FirebaseIncludeBaseAGP7)
            {
                writerHelper.Delete(dep);
            }
            writerHelper.Dispose();
        ...
        public List<AndroidGradleContext> GetGradleContext()
        {
            ...
            if (XDGCommonGradleProvider.IsNeedFirebaseDep(configMd))
            {
                ...
                var firebaseClassPathAgp7Dependencies = new AndroidGradleContext
                {
                    locationType = AndroidGradleLocationType.Custom,
                    locationParam = @"id 'com.android.library' version '\d+.\d+.\d+' apply false",
                    templateType = CustomTemplateType.BaseGradle,
                    unityVersionCompatibleType = UnityVersionCompatibleType.EveryVersion,
                    processContent = new List<string>(FirebaseIncludeBaseAGP7)
                };
                result.Add(firebaseClassPathAgp7Dependencies);                
#else
                var firebaseClassPathDependencies = new AndroidGradleContext();
                firebaseClassPathDependencies.locationType = AndroidGradleLocationType.Custom;
                firebaseClassPathDependencies.locationParam = @"classpath 'com.android.tools.build:gradle:\d+.\d+.\d+'";
