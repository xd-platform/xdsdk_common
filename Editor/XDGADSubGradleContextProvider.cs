        ...
        private List<AndroidGradleContext> GetGradleContextByXDConfig()
        {
            ...
            if (configObject["ad_config"] != null)
            {
                if (configObject["ad_config"]["tt_config"] != null)
                {
                    // 今日头条广告包 SDK
                    // Applog 上报组件（必须）
                    thirdPartyDeps.processContent.Add(@"    implementation 'com.bytedance.applog:RangersAppLog-Lite-cn:6.16.3'");
                    // 商业化组件（必须）6.14.2 及以上版本商业化组件独立，注：必须接入，以免平台限制应用上传
                    thirdPartyDeps.processContent.Add(@"    implementation 'com.bytedance.ads:AppConvert:1.3.0.4'");
                }
