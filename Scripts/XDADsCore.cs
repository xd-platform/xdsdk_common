    ...
    public class XDADsCore : IXDADs
    {
        ...
        public void GetADsSDKVersion(Action<string> callback)
        {
            ...
            EngineBridge.GetInstance().CallHandler(command, result => {
                XDGTool.Log("GetADsSDKVersion result --> " + JsonUtility.ToJson(result));
                if (!CheckResultSuccess(result))
                {
                    XDGTool.LogError($"GetADsSDKVersion failed: {result.message}");
                    callback("");
                    return;
                }
        ...
        public void GetChannelName(Action<string> callback)
        {
            var command = new Command.Builder()
                ...
                .CommandBuilder();
            EngineBridge.GetInstance().CallHandler(command, result => {
                XDGTool.Log("GetChannelName result --> " + JsonUtility.ToJson(result));
                if (!CheckResultSuccess(result))
                {
                    XDGTool.LogError($"GetChannelName failed: {result.message}");
                    callback("");
                    return;
                }
                callback(result.content);
