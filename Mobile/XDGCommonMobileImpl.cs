...
namespace  XD.SDK.Common{
    public class XDGCommonMobileImpl{
        private static readonly string COMMON_MODULE_UNITY_BRIDGE_NAME = "XDGCoreService";
        private static volatile XDGCommonMobileImpl _instance;
        private static readonly object locker = new object();

        private static string Channel = "";
        private static string GameVersion = "";

        private XDGCommonMobileImpl(){
            XDGLogger.Debug("===> Init XDG Bridge Service");
            EngineBridge.GetInstance()
                .Register(XDGUnityBridge.COMMON_SERVICE_NAME, XDGUnityBridge.COMMON_SERVICE_IMPL);
        }

        public static XDGCommonMobileImpl GetInstance(){
            if (_instance != null) return _instance;
            lock (locker){
                if (_instance == null){
                    _instance = new XDGCommonMobileImpl();
                }
            }
        ...
        public void IsInitialized(Action<bool> callback){
            var command = new Command.Builder()
                ...
                .CommandBuilder();
            EngineBridge.GetInstance().CallHandler(command, result => {
                XDGLogger.Debug("===> IsInitialized: " + result.ToJSON());
                if (!checkResultSuccess(result)){
                    callback(false);
                    return;
                }
        ...
        public void Share(ShareFlavors shareFlavors, string imagePath, XDGShareCallback callback){
            var command = new Command.Builder()
                ...
                .CommandBuilder();
            EngineBridge.GetInstance().CallHandler(command, result => {
                XDGLogger.Debug("===> share with type: " + shareFlavors + " result: " + result.ToJSON());
                if (!checkResultSuccess(result)){
        ...
        public void Report(string serverId, string roleId, string roleName){
            ...
            var command = new Command.Builder()
                ...
                .CommandBuilder();
            EngineBridge.GetInstance().CallHandler(command);
            XDGLogger.Debug($"===> Report:  {serverId} -- {roleId} -- {roleName}");
        }
        ...
        public void TrackRole(string serverId, string roleId, string roleName, int level){
            ...
            var command = new Command.Builder()
                ...
                .CommandBuilder();
            EngineBridge.GetInstance().CallHandler(command);
            XDGLogger.Debug($"===> TrackRole:  {serverId} -- {roleId} -- {roleName} -- {level}");
        }
        ...
        public void TrackUser(string userId){
            var command = new Command.Builder()
                ...
                .CommandBuilder();
            EngineBridge.GetInstance().CallHandler(command);
            XDGLogger.Debug($"===> TrackUser:  {userId}");
        }
        ...
        public void EventCompletedTutorial(){
            var command = new Command.Builder()
                ...
                .CommandBuilder();
            EngineBridge.GetInstance().CallHandler(command);
            XDGLogger.Debug("===> eventCompletedTutorial");
        }
        ...
        public void EventCreateRole(){
            var command = new Command.Builder()
                ...
                .CommandBuilder();
            EngineBridge.GetInstance().CallHandler(command);
            XDGLogger.Debug("===> eventCreateRole");
        }
        ...
        public void GetVersionName(Action<string> callback){
            var command = new Command.Builder()
                ...
                .CommandBuilder();
            EngineBridge.GetInstance().CallHandler(command, result => {
                XDGLogger.Debug("===> GetVersionName: " + result.ToJSON());
                if (!checkResultSuccess(result)){
        ...
        public void TrackAchievement(){
            var command = new Command.Builder()
                ...
                .CommandBuilder();
            EngineBridge.GetInstance().CallHandler(command);
            XDGLogger.Debug("===> trackAchievement");
        }
        ...
        public void SetCurrentUserPushServiceEnable(bool enable){
            var command = new Command.Builder()
                ...
                .CommandBuilder();
            EngineBridge.GetInstance().CallHandler(command);
            XDGLogger.Debug("===> SetCurrentUserPushServiceEnable");
        }
        ...
        public void IsCurrentUserPushServiceEnable(Action<bool> callback){
            var command = new Command.Builder()
                ...
                .CommandBuilder();
            EngineBridge.GetInstance().CallHandler(command, result => {
                XDGLogger.Debug("===> isCurrentUserPushServiceEnable: " + result.ToJSON());
                if (!checkResultSuccess(result)){
                    callback(false);
                    return;
                }
        ...
        public void GetRegionInfo(Action<XDGRegionInfoWrapper> callback){
            var command = new Command.Builder()
                ...
                .CommandBuilder();
            EngineBridge.GetInstance().CallHandler(command, result => {
                XDGLogger.Debug("GetRegionInfo result --> " + JsonUtility.ToJson(result));
        ...
        public void GetDid(Action<string> callback){
            var command = new Command.Builder()
                ...
                .CommandBuilder();
            EngineBridge.GetInstance().CallHandler(command, result => {
                XDGLogger.Debug("===> getDid: " + result.ToJSON());
                if (!checkResultSuccess(result)){
        ...
        public void GetAgreementList(Action<List<XDGAgreement>> callback)
        {
            try
            {
                var command = new Command.Builder()
                    ...
                    .CommandBuilder();
                EngineBridge.GetInstance().CallHandler(command, result => {
                    XDGLogger.Debug("===> GetAgreementList: " + result.ToJSON());
                    if (!checkResultSuccess(result)){
                        return;
                    }
        ...
        {
            XDGLogger.Debug("[GetAgreementList] jsonStr:\n" + jsonStr);
            List<XDGAgreementMobile> result = null;
            var dicStrStr = TapTap.Common.Json.Deserialize(jsonStr);
        ...
        public void ShowDetailAgreement(string agreementUrl)
        {
            try
            {
                var command = new Command.Builder()
                    ...
                    .CommandBuilder();
                EngineBridge.GetInstance().CallHandler(command);
                XDGLogger.Debug($"===> ShowDetailAgreement:  {agreementUrl}");
            }
            catch (Exception e)
            {
        ...
        public void SetExitHandler(Action onExitHandler)
        {
            try
            {
                XDGLogger.Debug("[unity 设置 exitHandler:");
                var command = new Command.Builder()
                    .Service(COMMON_MODULE_UNITY_BRIDGE_NAME)
                    ...
                    .CommandBuilder();
                EngineBridge.GetInstance().CallHandler(command, result => {
                    XDGLogger.Debug("===> setExitHandler: " + result.ToJSON());
                    if (!checkResultSuccess(result)){
                        return;
                    }
