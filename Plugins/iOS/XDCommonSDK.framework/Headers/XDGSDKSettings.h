...
/// 检测 debug 模式缓存，如果上次开启 debug 模式，则继续开启
+ (void)checkDebugModeCache;
...
/// 设置是否使用韩国区特殊的协议UI
/// @param enable 是否打开，默认 NO
+ (void)setSpecialKRAgreementUI:(BOOL)enable;

/// 设置隐藏用户中心的登出按钮
/// @param hide 隐藏，默认 NO
+ (void)setHideLogoutButton:(BOOL)hide;
