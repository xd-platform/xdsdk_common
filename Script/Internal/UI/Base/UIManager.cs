...
namespace XD.SDK.Common.PC.Internal {
    ...
    public class UIManager : MonoBehaviour {
        ...
        public static void ShowLoading(bool openNestedMask = false){
            Instance.loading.transform.SetAsLastSibling();
            Instance.loading.SetActive(true);
            Instance.UpdateMask();
            Instance.loadingBackground.SetActive(openNestedMask);
        }
        ...
        internal static UIManager Instance {
            get {
                if (instance == null && Application.isPlaying) {
                    ...
                    instance.loadingBackground = instance.loading.transform.Find("Background").gameObject;
                    instance.loadingBackground.gameObject.SetActive(false);
                }
        ...
        internal void DelayAction(float delayTime, Action action)
        {
            if (delayTime > 0)
            {
                StartCoroutine(InternalDelayAction(delayTime, action));
            }
        }
        
        private IEnumerator InternalDelayAction(float waitTime, Action action)
        {
