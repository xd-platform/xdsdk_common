...
namespace XD.SDK.Common.PC.Internal {
    ...
    internal partial class AliyunTrack
    {
        ...
        internal static void LogEventAsync(string eventName, Dictionary<string, string> contents)
        {
            ...
            {
                XDLogger.Warn($"[XD::LogEventAsync] TrackAsync Event Error! Message: {e.Message}\n{e.StackTrace}");
            }
        }
        ...
        private static async Task SendTrackInfo()
        {
            ...
            while (true)
            {
                if (requestList.Count > 0)
                {
                    ...
                    {
                        XDLogger.Warn(string.Format($"[XD::TrackAsync] request failed! Message: {e.Message}\n {e.StackTrace}"));
                        requestList.RemoveAt(0);
                    }
                }
                else
