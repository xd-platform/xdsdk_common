...
namespace XD.SDK.Common.PC.Internal {
    public class UserModule {
        // 获取用户信息
        private readonly static string XDG_USER_PROFILE = "api/account/v2/info";
        ...
        // 取消注销申请
        public readonly static string XDG_CANCEL_DELETE = "api/account/v1/cancel-delete";
        ...
        public static async Task<XDGUser> GetLocalUser() {
            if (current == null) {
                current = await persistence.Load<XDUser>(SystemInfo.deviceUniqueIdentifier);
            }
            return current;
        }
        ...
        public static async Task<XDGUser> Login(XD.SDK.Account.LoginType loginType) {
            ...
            if (loginType == XD.SDK.Account.LoginType.Default) {
                ...
                try {
                    ...
                    if (e.HttpStatusCode == 401 || e.code == 40311) {
                        throw e;
                    }
                } catch (Exception e) {
                    XDLogger.Warn(e.Message);
        ...
        public static async Task<XDGUser> FetchUserInfo()
        {
            ...
            {
                if (xdException.code == 40311 && xdException.extra_data != null)
                {
                    var title = xdException.extra_data["title"] as string;
                    var content = xdException.extra_data["content"] as string;
                    var negativeButton = xdException.extra_data["negativeButton"] as string;
                    var positiveButton = xdException.extra_data["positiveButton"] as string;
        ...
        private static async Task CancelDelete()
        {
            ...
            try
            {
                await XDHttpClient.Post<BaseResponse>(XDG_CANCEL_DELETE);
                UIManager.DismissLoading();
            }
            catch (Exception e)
            {
                UIManager.DismissLoading();
                var errorMsg = Localization.GetCurrentLocalizableString().NetworkErrorSafeRetry;
                if (e is XDException xdException && !string.IsNullOrEmpty(xdException.error_msg))
                {
                    errorMsg = xdException.error_msg;      
                }
